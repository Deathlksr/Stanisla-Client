...
const vec3 glowColor = vec3( .9, .638765, .15 );
const vec3 frontColor = vec3( 1., 1., 1. );
const vec3 frontColor2 = vec3( 1.0, .0, .5);
...
void main( void ) {
    ...
    // Модифицированная логика времени для добавления реверса
    float cycleTime = mod(time * 1.3, 6.0); // Удваиваем цикл до 6 секунд
    float t;

    if (cycleTime <= 3.0) {
        // Нормальное воспроизведение (0-3 секунды)
        t = cycleTime;
    } else {
        // Реверс (3-6 секунды)
        t = 6.0 - cycleTime;
    ...
    for (float i = 0.; i < 1.; i++) {
        float cycleTime2 = mod(time * 1.2 - i * 0.02, 6.0);
        float ti;
